...
\newcommand{\idx}[2]{\index{|\tikzmark{defAbefore}|#2|\tikzmark{defAafter}|}\textcolor{purple}{|\tikzmark{defBbefore}|#1|\tikzmark{defBafter}|}}|\tikzmark{eol}|
  \end{minted}
  
  \vspace{5mm}
  \textbf{Use:}
  \begin{minted}[breaklines,escapeinside=||]{latex}
We need to fit this to the \idx{|\tikzmark{useAbefore}|tradeoff|\tikzmark{useAafter}|}{|\tikzmark{useBbefore}|Tradeoff|\tikzmark{useBafter}|}.
  \end{minted}
  \pause
  \begin{tikzpicture}[remember picture]
    ...
    \coordinate (defA_coord) at ([yshift=-1.5mm] $(pic cs:defAbefore)!0.5!(pic cs:defAafter)$);
    \coordinate (defB_coord) at ([yshift= 3mm]   $(pic cs:defBbefore)!0.5!(pic cs:defBafter)$);
    \coordinate (useA_coord) at ([yshift=-1.5mm] $(pic cs:useAbefore)!0.5!(pic cs:useAafter)$);
    \coordinate (useB_coord) at ([yshift= 3mm]   $(pic cs:useBbefore)!0.5!(pic cs:useBafter)$);
...
\begin{frame}[fragile]
  ...
  \textbf{Definition:}
  \begin{minted}[breaklines,escapeinside=||]{latex}
\newcommand{\filename}[1]{ \textcolor{purple}{\texttt{|\tikzmark{defbefore}|#1|\tikzmark{defafter}|}} }
  \end{minted}
  
  \vspace{5mm}
  \textbf{Use:}
  \begin{minted}[breaklines,escapeinside=||]{latex}
The configuration is pulled from the \filename{|\tikzmark{usebefore}|conf.yaml|\tikzmark{useafter}|} file.
  \end{minted}
  \pause
  \begin{tikzpicture}[remember picture]
    \tikzstyle{arrow} = [thick,->,>=stealth, draw=black]
    ...
    \coordinate (def_coord) at ([yshift=-1.5mm] $(pic cs:defbefore)!0.5!(pic cs:defafter)$);
    \coordinate (use_coord) at ([yshift= 3mm]   $(pic cs:usebefore)!0.5!(pic cs:useafter)$);
