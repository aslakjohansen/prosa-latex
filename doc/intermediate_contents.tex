...
\begin{frame}[fragile]
  ...
  \pause
  \vspace{5mm}
  \textbf{A:} It is a bit like a function, but the evaluation of the body is done by replacing all occurrences of the parameters with the concrete arguments. The resulting text is then inserted in the macro calls place.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item This material is placed at the \textcolor{purple}{\textsl{anchor point}} of the nodes shape.
%    \item The shape itself is drawn according to a \textsl{style}.
    ...
    \item All of this is \textsl{drawn} according to a \textcolor{purple}{\textsl{style}}.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \vspace{3mm}
  Write a script that loads the spreadsheet, crunches the data and writes a \filename{.tex} file that makes use of a set of definitions from another \filename{.tex} file.
...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
    ...
    \pause \item Parse the GraphML file using the regular expressions.
          \\
          \pause \textbf{Note:} This is obviously \underline{so} the wrong approach \pause\ldots\ but it works.
...
\includeSVG{vector5}
  ...
  \pause
  \vspace{5mm}
  And then have a \textcolor{blue}{\href{https://github.com/aslakjohansen/prosa-latex/blob/main/bin/generate-tex-includes}{script}} crawl through the file inclusion tree of the root \LaTeX\ document to find all references to this macro, and automatically convert each of them from SVG to PDF.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \vspace{5mm}
  More info: \textcolor{blue}{\url{https://katex.org}}
...
\begin{frame}[fragile]
  ...
  \vspace{3mm}
  Proper TeX programming: State, loops, state \ldots
...
\begin{frame}[fragile]
  ...
  \vspace{1mm}
  \begin{enumerate}
    \descitem{Color Cube:} Use \TikZ\ to draw an RGB color cube that is centered on the page. It should have nodes for red, green, blue, black, cyan, magenta, yellow and white, and the appropriate edges.
