...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \descitem{Collaborator Skill Set} Often, your collaborators won't be familiar with \LaTeX, and then it simply isn't on the path of least resistance.
...
\begin{frame}
  \vspace{25mm}
  \begin{center}
    \Huge{Part 2\pause\\Building the PDF}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  Options:
  \begin{itemize}
    \pause \descitem{pdflatex} The old engine that is fast but lacks native unicode and modern OpenType font support. These are not serious restrictions.
    \pause \descitem{xelatex} A more modern version of \textdesc{pdflatex} that fixes its downsides. But on the flipside it is slower and has not been actively developed since 2017.
    \pause \descitem{lualatex} The most modern engine is scriptable through Lua. It also fixes the issues with \textdesc{pdflatex} and is under active development. It is even slower though.
      \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Paid tier provides GIT support.
  ...
  \begin{itemize}
    \item No GIT support.
    \item But if all else fails, you can pull the data out yourself.
  \end{itemize}
\end{frame}

\subsection{Building with Make}
\begin{frame}[fragile]
  \frametitle{Building with Make}
  \vspace{-1mm}
  \inputminted[fontsize=\tiny]{make}{Makefile}
\end{frame}

\subsection{Versioning with GIT}
\begin{frame}[fragile]
  \frametitle{Versioning with GIT}
  \vspace{3mm}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \begin{tikzpicture}[]
      ...
      \node[label,anchor=south] () at (\formatX, -0.5*\stepY) {\rotatebox{90}{Format}};
...
\begin{frame}
  \vspace{25mm}
  \begin{center}
    \Huge{Part 4\pause\\Auto-Generated Lists}
  \end{center}
\end{frame}
...
\begin{frame}
  \vspace{25mm}
  \begin{center}
    \Huge{Part 5\pause\\Dealing with Errors}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Abbreviations and glossaries: \textcolor{blue}{\url{https://sampo.website/blog/en/2023/latex-glossaries/}}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \pause
  \vspace{7mm}
  More material will appear at \textcolor{blue}{\url{https://latex.asjo.dk}}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \vspace{3mm}
  \begin{enumerate}
    \pause \descitem{New Project:} Create a new project (on Overleaf, GitHub or similar). Make sure that you have a report \LaTeX\ document that builds.
    \pause \descitem{Tabular Data:} Find some tabular data, place it in a \texttt{tabular} environment, and make it look pretty.
    \pause \descitem{Formula:} Pick any formula (e.g., the discrete fourier transformation), and express it though \LaTeX\ code.
    \pause \descitem{Report:} Pick any old report, and port it to \LaTeX. The point is not to go through the whole report, but rather to get a feel of (i) the effort needed to port an old report, and (ii) the perceived typographical quality per effort.
  \end{enumerate}
\end{frame}
